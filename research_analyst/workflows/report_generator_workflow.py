...
class AutonomousReportGenerator:
    ...
    def create_analyst(self, state: GenerateAnalystsState):
        ...
        try:
            ...
            # Get format instructions from the structured output
            format_instructions = "Return a JSON object with 'analysts' field containing a list of analyst objects. Each analyst should have: 'affiliation', 'name', 'role', and 'description' fields."
            
            system_prompt = CREATE_ANALYSTS_PROMPT.format(
                topic=topic, 
                max_analysts=max_analysts,
                human_analyst_feedback=human_analyst_feedback,
                analysts="",  # Add empty analysts since it's referenced in the prompt
    ...
    def human_feedback(self, state: ResearchGraphState):
        """Process human analyst feedback and regenerate analysts if feedback provided."""
        try:
            human_analyst_feedback = state.get("human_analyst_feedback", "")
            existing_analysts = state.get("analysts", [])
            self.logger.info("Processing human feedback", 
                           feedback=human_analyst_feedback, 
                           existing_analyst_count=len(existing_analysts))
            
            # If feedback is provided, regenerate analysts with the feedback
            if human_analyst_feedback and human_analyst_feedback.strip():
                self.logger.info("Regenerating analysts based on feedback")
                # Create a GenerateAnalystsState for the create_analyst function
                analyst_state = {
                    "topic": state.get("topic", ""),
                    "max_analysts": state.get("max_analysts", 3),
                    "human_analyst_feedback": human_analyst_feedback,
                    "analysts": existing_analysts
                }
                result = self.create_analyst(analyst_state)
                new_analysts = result.get("analysts", [])
                self.logger.info("Generated new analysts", count=len(new_analysts))
                return {"analysts": new_analysts}
            else:
                self.logger.info("No feedback provided, continuing with existing analysts", 
                               count=len(existing_analysts))
                return {"analysts": existing_analysts}
        except Exception as e:
    ...
    def initiate_all_interviews(self, state: ResearchGraphState):
        """Conditional function to determine if interviews should proceed."""
        topic = state.get("topic", "Untitled Topic")
        analysts = state.get("analysts", [])
        self.logger.info("Initiating interviews", topic=topic, analyst_count=len(analysts))
        if not analysts:
            self.logger.warning("No analysts found — skipping interviews")
            return END
        self.logger.info("Starting interviews with analysts", analysts=[a.name if hasattr(a, 'name') else "Unknown" for a in analysts])
        return [
            Send(
                "conduct_interview",
                {
